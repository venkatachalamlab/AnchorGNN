    ...
    def __init__(self, data: pd.DataFrame = None):
        ...
        if data is not None:
            ...
            for col in data.columns:
                if isinstance(data[col].dtype, np.dtype):
                    kind = data[col].dtype.kind
                    if kind in {"O", "V", "S", "U"}:
                        try:
                            # Decode raw byte strings if needed
                            data[col] = data[col].apply(
                                lambda x: x.tobytes().decode("utf-8", errors="ignore") if hasattr(x, "tobytes") else str(x)
                            )
                        except Exception:
        ...
        self.next_id = np.max(self.df.index) + 1 if not self.df.empty else 1
